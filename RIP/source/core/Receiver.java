			...
			while (!socket.isClosed()) {
				...
				if (!Inet4Address.getLocalHost().getHostAddress().equals(packet.getAddress().getHostAddress())) {
                    ...
                    for (int index = 0; index < array.size(); index++ ){
	                    ...
	                    String netWorkAddress = object.get("netWorkAddress").toString();
	                    String mask = object.get("mask").toString();
	                    String defaultGetWay = object.get("defaultGetWay").toString();
	                    String interf = object.get("interf").toString();
	                    int metric = Integer.parseInt(object.get("metric").toString());
                        service.dataHandler(netWorkAddress, mask, defaultGetWay, interf, metric);
                    }
				}
			}
		} catch (IOException exception) {
